...
bool FSR2FeatureVk212::InitFSR2(const NVSDK_NGX_Parameter* InParameters)
{
	...
	{
		spdlog::error("FSR2FeatureVk::InitFSR2 ffxGetInterfaceVK error: {0}", ResultToString212(errorCode));
		free(scratchBuffer);
		return false;
	}
	...
	{
		spdlog::error("FSR2FeatureVk::InitFSR2 ffxFsr2ContextCreate error: {0}", ResultToString212(ret));
		return false;
	}
