...
{
	spdlog::debug("FSR3FeatureDx12::Init");
...
{
	spdlog::debug("FSR3FeatureDx12::Evaluate");
...
{
	spdlog::error("FSR3Feature::ReInit");
	...
	if (IsInited())
	{
		...
		if (errorCode != FFX_OK)
			spdlog::error("FSR3Feature::ReInit ffxFsr2ContextDestroy error: {0:x}", errorCode);
