...
FT_Dx12::FT_Dx12(std::string InName, ID3D12Device* InDevice, DXGI_FORMAT InFormat) : _name(InName), _device(InDevice), format(InFormat)
{
    ...
    if (Config::Instance()->UsePrecompiledShaders.value_or(true))
    {
        ...
        if (InFormat == DXGI_FORMAT_R10G10B10A2_UNORM || InFormat == DXGI_FORMAT_R10G10B10A2_TYPELESS || 
            InFormat == DXGI_FORMAT_R16G16B16A16_FLOAT || InFormat == DXGI_FORMAT_R11G11B10_FLOAT ||
            InFormat == DXGI_FORMAT_R32G32B32A32_FLOAT || InFormat == DXGI_FORMAT_R32G32B32_FLOAT)
        {
            computePsoDesc.CS = CD3DX12_SHADER_BYTECODE(reinterpret_cast<const void*>(r10g10b10a2_cso), sizeof(r10g10b10a2_cso));
        }
        else if (InFormat == DXGI_FORMAT_R8G8B8A8_TYPELESS || InFormat == DXGI_FORMAT_R8G8B8A8_UNORM_SRGB || InFormat == DXGI_FORMAT_R8G8B8A8_UNORM)
        {
