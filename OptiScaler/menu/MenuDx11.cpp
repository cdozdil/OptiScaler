...
static void CleanupRenderTargetDx12(bool clearQueue)
{
    ...
    if (clearQueue)
    {
        if (MenuBase::IsInited() && g_pd3dDeviceParam != nullptr && g_pd3dSrvDescHeap != nullptr && ImGui::GetIO().BackendRendererUserData)
        {
...
static HRESULT Present(IDXGISwapChain* pSwapChain, UINT SyncInterval, UINT Flags, const DXGI_PRESENT_PARAMETERS* pPresentParameters, IUnknown* pDevice, HWND hWnd)
{
    ...
    else if (pDevice->QueryInterface(IID_PPV_ARGS(&cq)) == S_OK)
    {
        ...
        MenuDx11::GameCommandQueue = (ID3D12CommandQueue*)pDevice;
    ...
    if (MenuDx11::dx12UpscaleTrig && MenuDx11::readbackBuffer != nullptr && MenuDx11::queryHeap != nullptr && cq != nullptr)
    {
        if (MenuBase::IsInited() && MenuBase::IsVisible())
        {
            UINT64* timestampData;
            MenuDx11::readbackBuffer->Map(0, nullptr, reinterpret_cast<void**>(&timestampData));
            ...
            // Unmap the buffer
            MenuDx11::readbackBuffer->Unmap(0, nullptr);
        }

        MenuDx11::dx12UpscaleTrig = false;
    }
    else if (MenuDx11::dx11UpscaleTrig[MenuDx11::currentFrameIndex] && device != nullptr && MenuDx11::disjointQueries[0] != nullptr &&
             MenuDx11::startQueries[0] != nullptr && MenuDx11::endQueries[0] != nullptr)
    {
        ...
        if (MenuBase::IsInited() && MenuBase::IsVisible())
        {
            ...
            if (g_pd3dDeviceContext->GetData(MenuDx11::disjointQueries[MenuDx11::previousFrameIndex], &disjointData, sizeof(disjointData), 0) == S_OK)
            {
                if (!disjointData.Disjoint && disjointData.Frequency > 0)
                {
                    UINT64 startTime = 0, endTime = 0;
                    if (g_pd3dDeviceContext->GetData(MenuDx11::startQueries[MenuDx11::previousFrameIndex], &startTime, sizeof(UINT64), 0) == S_OK &&
                        g_pd3dDeviceContext->GetData(MenuDx11::endQueries[MenuDx11::previousFrameIndex], &endTime, sizeof(UINT64), 0) == S_OK)
                    {
