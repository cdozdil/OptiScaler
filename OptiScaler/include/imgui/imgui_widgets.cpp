...
void ImGui::Image(ImTexture texture, const ImVec2& image_size, const ImVec2& uv0, const ImVec2& uv1,
...
// ImageButton() API.
bool ImGui::ImageButtonEx(ImGuiID id, ImTexture texture, const ImVec2& image_size, const ImVec2& uv0, const ImVec2& uv1,
                                    ...
                                    const char** remaining, ImVec2* out_offset, bool stop_on_new_line)
{
    ...
    while (s < text_end)
    {
        ...
        const float char_width =
            ((int) c < font->IndexAdvanceX.Size ? font->IndexAdvanceX.Data[c] : font->FallbackAdvanceX) * scale;
...
static void STB_TEXTEDIT_LAYOUTROW(StbTexteditRow* r, ImGuiInputTextState* obj, int line_start_idx)
{
    ...
    const ImVec2 size =
        InputTextCalcTextSize(obj->Ctx, text + line_start_idx, text + obj->CurLenA, &text_remaining, NULL, true);
    r->x0 = 0.0f;
    r->x1 = size.x;
    ...
    if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id)
    {
        ...
        else if (is_cut || is_copy)
        {
            ...
            if (g.PlatformIO.Platform_SetClipboardTextFn != NULL)
            {
                ...
                const int ie =
                    state->HasSelection() ? ImMax(state->Stb->select_start, state->Stb->select_end) : state->CurLenA;
...
void ImGui::DebugNodeInputTextState(ImGuiInputTextState* state)
{
    ...
    Text("CurLenA: %d, Cursor: %d, Selection: %d..%d", state->CurLenA, stb_state->cursor, stb_state->select_start,
            ...
            (ms->RangeSrcPassedBy == false &&
             ms->IO.RangeSrcItem !=
                 ImGuiSelectionUserData_Invalid)) // Can't read storage->RangeSrcItem here -> we want the state at
                                                  // begining of the scope (see tests for easy failure)
        {
