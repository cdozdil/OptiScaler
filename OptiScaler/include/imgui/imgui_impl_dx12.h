...
IMGUI_IMPL_API void ImGui_ImplDX12_UpdateFontsTexture(); // This should be called when the font texture needs updating
                                                         // (if ImGuiBackendFlags_RendererHasTexReload was set), with
                                                         // the GPU idle. It will perform the update using a temporary
                                                         // command list, and block internally until it completes.
