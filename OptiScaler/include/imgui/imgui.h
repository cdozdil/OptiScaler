...
// - Note that Image() may add +2.0f to provided size if a border is visible, ImageButton() adds style.FramePadding*2.0f
// to provided size.
IMGUI_API void Image(ImTexture texture, const ImVec2& image_size, const ImVec2& uv0 = ImVec2(0, 0),
...
IMGUI_API void Image(ImTextureID user_texture_id, const ImVec2& image_size, const ImVec2& uv0 = ImVec2(0, 0),
                     const ImVec2& uv1 = ImVec2(1, 1), const ImVec4& tint_col = ImVec4(1, 1, 1, 1),
                     const ImVec4& border_col = ImVec4(0, 0, 0, 0));
IMGUI_API bool ImageButton(const char* str_id, ImTexture texture, const ImVec2& image_size,
...
enum ImGuiChildFlags_
{
    ...
    ImGuiChildFlags_NavFlattened = 1 << 8, // [BETA] Share focus scope, allow gamepad/keyboard navigation to cross over
...
enum ImGuiBackendFlags_
{
    ...
    ImGuiBackendFlags_RendererHasTexReload = 1 << 4 // Backend Renderer checks IsDirty() on the font atlas texture after
                                                    // EndFrame()/Render() and reupload the texture to GPU if required.
};
...
struct ImGuiIO
{
    ...
    bool ConfigDebugHighlightIdConflicts; // = true           // Highlight and show an error message when multiple items
...
struct ImTexture
{
    ...
    // inline friend bool operator==(const ImTexture& lhs, const ImTexture& rhs) { return memcmp(&lhs, &rhs,
    // sizeof(ImTexture)) == 0; }
    inline friend bool operator==(const ImTexture& lhs, const ImTexture& rhs)
    {
        return lhs.TextureId == rhs.TextureId && lhs.Type == rhs.Type && lhs.FontAtlasPage == rhs.FontAtlasPage;
    }
...
struct ImDrawCmd
{
    ...
    ImTexture Texture; // 8-12 // User-provided texture ID. Set by user in ImfontAtlas::SetTexID() for fonts or passed
    ...
    inline void SetTexID(ImTextureID tex_id)
    {
        Texture.TextureId = tex_id;
        Texture.Type = ImTextureType_UserID;
    }
...
struct ImDrawList
{
    ...
    IMGUI_API void AddImage(ImTexture texture, const ImVec2& p_min, const ImVec2& p_max,
    ...
    IMGUI_API void AddImage(ImTextureID user_texture_id, const ImVec2& p_min, const ImVec2& p_max,
                            const ImVec2& uv_min = ImVec2(0, 0), const ImVec2& uv_max = ImVec2(1, 1),
                            ImU32 col = IM_COL32_WHITE);
    IMGUI_API void AddImageQuad(ImTexture texture, const ImVec2& p1, const ImVec2& p2, const ImVec2& p3,
    ...
    IMGUI_API void AddImageQuad(ImTextureID user_texture_id, const ImVec2& p1, const ImVec2& p2, const ImVec2& p3,
                                ...
                                const ImVec2& uv3 = ImVec2(1, 1), const ImVec2& uv4 = ImVec2(0, 1),
                                ImU32 col = IM_COL32_WHITE);
    IMGUI_API void AddImageRounded(ImTexture texture, const ImVec2& p_min, const ImVec2& p_max, const ImVec2& uv_min,
...
{
    ImTextureFormat_Alpha8, // 1 component per pixel, each component is unsigned 8-bit. Total size = TexWidth *
                            // TexHeight
    ImTextureFormat_RGBA32 // 4 component per pixel, each component is unsigned 8-bit. Total size = TexWidth * TexHeight
                           // * 4
};
...
struct ImFontAtlas
{
    ...
    IMGUI_API bool IsDirty(); // Returns true if the font needs to be (re-)built. User code should call GetTexData***
                              // and update either the whole texture or dirty region as required.
...
struct ImFont
{
    ...
    ImVector<float> IndexAdvanceX; // 12-16 // out //            // Sparse. Glyphs->AdvanceX in a directly indexable way
                                   // (cache-friendly for CalcTextSize functions which only this info, and are often
...
IMGUI_API ImGuiKey GetKeyIndex(ImGuiKey key); // Map ImGuiKey_* values into legacy native key index. == io.KeyMap[key].
                                              // When using a 1.87+ backend using io.AddKeyEvent(), calling
                                              // GetKeyIndex() with ANY ImGuiKey_XXXX values will return the same value!
// static inline ImGuiKey GetKeyIndex(ImGuiKey key)                          { IM_ASSERT(key >= ImGuiKey_NamedKey_BEGIN
// && key < ImGuiKey_NamedKey_END); return key; }
