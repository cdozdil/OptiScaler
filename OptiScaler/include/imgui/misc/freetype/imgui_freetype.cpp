...
//  2023/11/13: added support for ImFontConfig::RasterizationDensity field for scaling render density without scaling
//  metrics. 2023/08/01: added support for SVG fonts, enable by using '#define IMGUI_ENABLE_FREETYPE_LUNASVG' (#6591)
//  2023/01/04: fixed a packing issue which in some occurrences would prevent large amount of glyphs from being packed
//  correctly. 2021/08/23: fixed crash when FT_Render_Glyph() fails to render a glyph and returns NULL. 2021/03/05:
//  added ImGuiFreeTypeBuilderFlags_Bitmap to load bitmap glyphs. 2021/03/02: set 'atlas->TexPixelsUseColors = true' to
//  help some backends with deciding of a preferred texture format. 2021/01/28: added support for color-layered glyphs
//  via ImGuiFreeTypeBuilderFlags_LoadColor (require Freetype 2.10+). 2021/01/26: simplified integration by using
//  '#define IMGUI_ENABLE_FREETYPE'. renamed ImGuiFreeType::XXX flags to ImGuiFreeTypeBuilderFlags_XXX for consistency
//  with other API. removed ImGuiFreeType::BuildFontAtlas(). 2020/06/04: fix for rare case where FT_Get_Char_Index()
//  succeed but FT_Load_Glyph() fails. 2019/02/09: added RasterizerFlags::Monochrome flag to disable font anti-aliasing
//  (combine with ::MonoHinting for best results!) 2019/01/15: added support for imgui allocators + added FreeType only
//  override function SetAllocatorFunctions(). 2019/01/10: re-factored to match big update in STB builder. fixed texture
//  height waste. fixed redundant glyphs when merging. support for glyph padding. 2018/06/08: added support for
//  ImFontConfig::GlyphMinAdvanceX, GlyphMaxAdvanceX. 2018/02/04: moved to main imgui repository (away from
//  http://www.github.com/ocornut/imgui_club) 2018/01/22: fix for addition of ImFontAtlas::TexUvscale member.
...
struct FontInfo
{
    ...
    float LineSpacing; // The baseline-to-baseline distance. Note that it usually is larger than the sum of the ascender
                       // and descender taken as absolute values. There is also no guarantee that no glyphs extend above
                       // or below subsequent baselines when using this distance. Think of it as a value the designer of
                       // the font finds appropriate.
...
{
    // Vuhdo: I'm not sure how to deal with font sizes properly. As far as I understand, currently ImGui assumes that
    // the 'pixel_height' is a maximum height of an any given glyph, i.e. it's the sum of font's ascender and descender.
    // Seems strange to me. NB: FT_Set_Pixel_Sizes() doesn't seem to get us the same result.
...
const FT_Glyph_Metrics* FreeTypeFont::LoadGlyph(uint32_t codepoint)
{
    ...
    IM_ASSERT(slot->format != FT_GLYPH_FORMAT_SVG &&
              "The font contains SVG glyphs, you'll need to enable IMGUI_ENABLE_FREETYPE_LUNASVG in imconfig.h and "
              "install required libraries in order to use this font");
#endif
    IM_ASSERT(slot->format == FT_GLYPH_FORMAT_OUTLINE || slot->format == FT_GLYPH_FORMAT_BITMAP);
...
bool ImFontAtlasBuildWithFreeTypeEx(FT_Library ft_library, ImFontAtlas* atlas, unsigned int extra_flags)
{
    ...
    // We allocate in chunks of 256 KB to not waste too much extra memory ahead. Hopefully users of FreeType won't mind
    ...
    if (atlas->TexDesiredWidth > 0)
        ...
        atlas->TexData.TexWidth = (surface_sqrt >= 4096 * 0.7f)   ? 4096
    ...
    stbrp_init_target(&pack_context, atlas->TexData.TexWidth - atlas->TexGlyphPadding,
        ...
        for (int glyph_i = 0; glyph_i < src_tmp.GlyphsCount; glyph_i++)
            if (src_tmp.Rects[glyph_i].was_packed)
                atlas->TexData.TexHeight =
                    ImMax(atlas->TexData.TexHeight, src_tmp.Rects[glyph_i].y + src_tmp.Rects[glyph_i].h);
    }

    // 7. Allocate texture
    atlas->TexData.TexHeight = (atlas->Flags & ImFontAtlasFlags_NoPowerOfTwoHeight)
                                   ? (atlas->TexData.TexHeight + 1)
                                   : ImUpperPowerOfTwo(atlas->TexData.TexHeight);
