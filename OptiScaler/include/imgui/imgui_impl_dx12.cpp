...
//  2023-XX-XX: *BREAKING CHANGE*: DirectX12: Changed ImGui_ImplDX12_CreateFontsTexture() to
//  ImGui_ImplDX12_UpdateFontsTexture(), which should be called at the start of the frame when
//  ImGui::GetIO().Fonts->IsDirty() is true to reupload the font texture. ImGuiBackendFlags_RendererHasTexReload should
//  be set once this is implemented. 2022-10-11: Using 'nullptr' instead of 'NULL' as per our switch to C++11.
...
void ImGui_ImplDX12_UpdateFontsTexture()
{
    ...
    // ImU64'.
    // - This is because we need ImTextureID to carry a 64-bit value and by default ImTextureID is defined as void*.
    // [Solution 1] IDE/msbuild: in "Properties/C++/Preprocessor Definitions" add 'ImTextureID=ImU64' (this is what we
    // do in the 'example_win32_direct12/example_win32_direct12.vcxproj' project file) [Solution 2] IDE/msbuild: in
    ...
    // Create the root signature
    {
        ...
        if (d3d12_dll == nullptr)
        {
            ...
            const char* localD3d12Paths[] = { ".\\d3d12.dll", ".\\d3d12on7\\d3d12.dll",
                                              ".\\12on7\\d3d12.dll" }; // A. current directory, B. used by some games,
                         ...
                         D3D12_GPU_DESCRIPTOR_HANDLE font_srv_gpu_desc_handle)
{
    ...
    io.BackendFlags |= ImGuiBackendFlags_RendererHasVtxOffset; // We can honor the ImDrawCmd::VtxOffset field, allowing
                                                               // for large meshes.
    // Note that we explicitly do *not* set ImGuiBackendFlags_RendererHasTexReload here, because in DX12 that requires
