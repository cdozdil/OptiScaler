...
//              need to call this.
//  2023-XX-XX: *BREAKING CHANGE*: Vulkan: Changed ImGui_ImplVulkan_CreateFontsTexture() to
//  ImGui_ImplVulkan_UpdateFontsTexture(), which should be called at the start of the frame when
//  ImGui::GetIO().Fonts->IsDirty() is true to reupload the font texture. ImGuiBackendFlags_RendererHasTexReload should
//  be set once this is implemented. 2023-07-04: Vulkan: Added optional support for VK_KHR_dynamic_rendering. User needs
//  to set init_info->UseDynamicRendering = true and init_info->ColorAttachmentFormat. 2023-01-02: Vulkan: Fixed sampler
//  passed to ImGui_ImplVulkan_AddTexture() not being honored + removed a bunch of duplicate code. 2022-10-11: Using
//  'nullptr' instead of 'NULL' as per our switch to C++11. 2022-10-04: Vulkan: Added experimental
...
bool ImGui_ImplVulkan_UpdateFontsTexture()
{
    ...
    if ((!bd->FontView) || (bd->FontTextureWidth != width) || (bd->FontTextureHeight != height))
    {
        ...
        // } if (bd->FontImage) { vkDestroyImage(v->Device, bd->FontImage, v->Allocator); bd->FontImage =
        // VK_NULL_HANDLE; } if (bd->FontMemory) { vkFreeMemory(v->Device, bd->FontMemory, v->Allocator); bd->FontMemory
        // = VK_NULL_HANDLE; }
    ...
    // Copy to Image:
    {
        ...
        vkCmdPipelineBarrier(bd->FontCommandBuffer, VK_PIPELINE_STAGE_HOST_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, 0, 0,
...
bool ImGui_ImplVulkan_Init(ImGui_ImplVulkan_InitInfo* info)
{
    ...
    io.BackendFlags |= ImGuiBackendFlags_RendererHasVtxOffset; // We can honor the ImDrawCmd::VtxOffset field, allowing
                                                               // for large meshes.
    // Note that we explicitly do *not* set ImGuiBackendFlags_RendererHasTexReload here, because in Vulkan that requires
    // support in the caller as well, so we leave setting it (or not) up to that code.
                                             ...
                                             int w, int h, uint32_t min_image_count)
{
    ...
    // Create Swapchain
    {
        ...
        wd->FrameSemaphores = (ImGui_ImplVulkanH_FrameSemaphores*) IM_ALLOC(sizeof(ImGui_ImplVulkanH_FrameSemaphores) *
                                                                            wd->SemaphoreCount);
